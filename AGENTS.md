...
| `npm test` | ✅ 62/62 lulus (7 berkas) — diperbarui audit 2026.17 |
| `npm run typecheck` | ✅ exit 0 |
| `npm run build` | ✅ exit 0 — 214 halaman statis + studi (audit 2026.17) |
...
- ~~**GITHUB_TOKEN di Vercel belum terverifikasi bekerja**~~ — ✅ **SELESAI 22 Sep 2026** (`9c820d3`): akar masalah adalah env `GITHUB_OWNER` berisi string kosong → `??` tidak menggantikan → fetch 404. Fix: `?.trim() ||`. Sekarang `live: true`, `source: github-api`. Token fine-grained terpasang di Production + Preview (nilai TIDAK dicatat di repo — jangan tulis fragmen token apa pun di docs publik).
...
- **Deployment "DOWN" di /status bisa berarti pause disengaja (22 Sep 2026):** pemilik sengaja me-pause sejumlah proyek di Vercel (mis. kune-ya, watchdog-mata). Status DOWN yang ditampilkan MONITOR itu benar (situs memang tidak aktif) — BUKAN insiden dan jangan dilaporkan sebagai outage.
...
- **i18n 2026.16**: kamus 474 kunci paritas penuh; SEMUA copy UI lewat `<T>`/`t()` (PaymentModal, ErrorBoundary, OfflineCacheList, CategoryDonut, Skeletons, tooltip RepoGrid); `TitleSync` menyetel `document.title` per locale di 13 rute; overlay `data/repo-descriptions.en.json` lengkap (hanya deskripsi ID yang di-overlay, EN-native pass-through — jangan tambah entri untuk deskripsi yang sudah EN). Istilah universal (fork/live/demo/Ctrl K) sengaja keras.
- **A11y CI 2026.16**: `npm run test:a11y` = `tests/e2e/a11y.mjs` (axe-core + Playwright, 11 rute + 2 EN dengan verifikasi `<html lang>`; serious/critical gagal, wcag22aa termasuk target-size). Baseline: 13/13 lolos 0 pelanggaran. Chromium butuh `npx playwright install chromium --with-deps`.
- **Bukti CSP untuk enforce**: laporan `/api/csp-report` menunjukkan ping demo `/system` diblokir `connect-src` — **enforce tanpa menambah URL demo ke connect-src akan mematahkan pemeriksaan live /system**.
- **`docs/AUDIT-2026.17.md`** (23 Sep 2026): audit menyeluruh + peta jalan pasca-Fase 2 — posisi terverifikasi, inventaris tertunda 4 kategori (aksi pemilik / jendela waktu / upstream / antrian kode), urutan kerja Langkah 1–6 dengan kebutuhan lengkap. Temuan kunci: **fiber 9.8.0 peer `<19.4` → T3 terbuka**; pembayaran belum dikonfigurasi (midtrans/stripe false); sw.js versi manual (BUILD_ID auto-bump belum diimplementasi).
